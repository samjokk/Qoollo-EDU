  ...
  <div class="projects-page">
    ...
    <div class="content">
      <div class="best-projects">
        ...
        <TopProjects
          :cards="topProjects"
          :projectEventFlag="true"
          class="best-projects-list"
        />
      </div>
      <div class="projects">
        <div class="search projects-item">
          <p>Найди свой свой проект и присоединяйся!</p>
          <div class="searchline">
            ...
            <label class="searchlinelabel">
              <input
                ...
                v-on:keyup.enter="findProject($event)"
            /></label>
          </div>
        </div>
        <ListProjects
          ...
          :cards="projects"
          :projectEventFlag="true"
          class="projectsList projects-item"
        />
      </div>
      <div class="filters">
        <router-link to="/createproject">
          <Button
            ...
            :textButton="'Запуск своего проекта'"
            :width="'342px'"
            :height="'78px'"
          />
        </router-link>
        ...
        <div class="block-filters">
          ...
          <div class="status">
            <label class="radio">
              <input type="radio" name="project" value="open" />
              <div class="radio-text">Открытый проект</div>
            </label>
            <label class="radio">
              <input type="radio" name="project" value="close" />
              <div class="radio-text">Закрытый проект</div>
            </label>
          </div>
          <div class="tags">
            <div class="title">
...
<script>
import TopProjects from "@/components/Cards/TopProjectsEvents.vue";
import ListProjects from "@/components/ListProjectEvent.vue";
import Button from "@/components/Buttons/ButtonProject.vue";

let checkProjects = true;
let allProjectsHTML;
let copyAllProjectsHTML;

async function getDataProject() {
  const res = await fetch(`http://localhost:5000/api/search/projects`, {
    method: "GET",
    // headers: {
    //     Authorization: token
  });
...
export default {
  ...
  methods: {
    ...
    getFilters() {
      ...
      let filterProject = document.getElementsByName("project"); //Поле пока не используется
      //Проверка общая - хз понадобится ли
      // let radioChecked = false;
      // for (let i = 0; i < filtersLikes.length; i++) {
      //   if (filtersLikes[i].checked || filtersProject[i].checked)
      //     radioChecked = true;
      // }

      allProjectsHTML = copyAllProjectsHTML;
      this.showAllProjects();
      if (tags.length != 0) this.sortProjectsByTags(tags);
      else this.showAllProjects();

      if (filterLikes[0].checked) this.sortProjectsAscendingDescending(true);
      else if (filterLikes[1].checked)
        this.sortProjectsAscendingDescending(false);
    },
    findProject(event) {
      this.checkAllProjectsHTML();

      if (event.target.value != "") {
        for (let li in this.projects) {
          if (
            !this.projects[li].name
              .toLowerCase()
              .includes(event.target.value.toLowerCase()) &&
            !allProjectsHTML[li].classList.contains("hidden")
          ) {
            allProjectsHTML[li].classList.add("hidden");
          } else if (
            this.projects[li].name
              .toLowerCase()
              .includes(event.target.value.toLowerCase()) &&
            ...
            allProjectsHTML[li].classList.remove("hidden");
          }
        }
      } else {
        this.showAllProjects();
    ...
    sortProjectsByTags(tags) {
      for (let i = 0; i < allProjectsHTML.length; i++) {
        let tagsProject = new Array();
        for (let tag in this.projects[i].tags) {
          tagsProject.push(this.projects[i].tags[tag].id);
        }
        if (!this.checkConvergenceTags(tags, tagsProject)) {
          if (!allProjectsHTML[i].classList.contains("hidden"))
            allProjectsHTML[i].classList.add("hidden");
        }
      }
    },
    sortProjectsAscendingDescending(ascOrDes) {
      let sortedProjectsHTML = document.getElementsByClassName(
        "projectsList"
      )[0];
      ...
      for (let i = 0; i < allProjectsHTML.length; i++) {
        listId.push(i);
        listRating.push(this.projects[i].rating);
        sortedProjectsHTML.removeChild(allProjectsHTML[i]);
      }
      ...
      if (ascOrDes)
        for (let i = 0; i < listId.length; i++)
          sortedProjectsHTML.append(allProjectsHTML[listId[i]]);
      else if (!ascOrDes)
        for (let i = 0; i < listId.length; i++)
          sortedProjectsHTML.prepend(allProjectsHTML[listId[i]]);

      allProjectsHTML = sortedProjectsHTML.querySelectorAll("li");
    },
    checkAllProjectsHTML() {
      if (checkProjects) {
        allProjectsHTML = document
          .getElementsByClassName("projectsList")[0]
          .querySelectorAll("li");
        copyAllProjectsHTML = allProjectsHTML;
        checkProjects = false;
      }
    },
    showAllProjects() {
      for (let li in this.projects)
        if (allProjectsHTML[li].classList.contains("hidden"))
          allProjectsHTML[li].classList.remove("hidden");
    },
    checkConvergenceTags(tags, tagsProject) {
      let convergence = true;
      for (let j = 0; j < tags.length; j++) {
        if (!tagsProject.includes(tags[j])) convergence = false;
      }
      return convergence;
    },
    async getData() {
      ...
      res = await res.json();
      this.projects = await res.projects;
      this.tags = await res.tags;
      this.topProjects = await res.topProjects;
  ...
  created() {
    this.getData();
    this.$root.$refs.header.printMenu(2);
  },
};
</script>
