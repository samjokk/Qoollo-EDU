...
namespace QoolloEDU.Database.Repositories.DeveloperRepository
{
    public class DeveloperRepository: IDeveloperRepository
    {
        ...
        public DeveloperRepository(QoolloEduDbContext context)
        {
            _context = context;
        ...
        public async Task<UserDto> GetDeveloperAsync(int developerId)
        {
            ...
            return new UserDto()
            {
                ...
                Age = DateTime.Now.DayOfYear > developer.BirthDate.DayOfYear ? 
                    DateTime.Now.Year - developer.BirthDate.Year + 1 :
                    DateTime.Now.Year - developer.BirthDate.Year,
                About = developer.BaseUser.About,
                Nickname = developer.Nickname,
                Photo = developer.BaseUser.Photo,
            };
        }
        
        public async Task<UserMiniDto> GetUserAsync(int developerId)
        {
            var developer = await _context.Developer
                .Include(d => d.BaseUser)
                .FirstOrDefaultAsync(d => d.Id == developerId);
        ...
        public async Task<List<AchievementDto>> GetDeveloperAchievementsAsync(int developerId)
        {
            var projects = await _context.DeveloperProject
                .Include(dp => dp.Project)
                .Include(dp => dp.Project.Event)
                .Where(dp => dp.DeveloperId == developerId)
                .Where(dp => dp.Project.EventId != null)
                .Where(dp => dp.Project.Place <= PlaceType.Third)
                .OrderBy(dp => dp.Project.Place)
                .ToListAsync();
        
            return projects.Select(project => new AchievementDto()
            {
                ProjectId = project.ProjectId,
                ProjectName = project.Project.Name,
                // ReSharper disable once PossibleNullReferenceException
                // ReSharper disable once PossibleInvalidOperationException
                Place = (int) project.Project.Place,
            }).ToList();
        }
        ...
        public async Task<RegistrationDto> CreateAsync(UserRegDto userRegDto)
        {
            var emailCheck = await _context.BaseUser
                .Where(du => du.Email == userRegDto.Email)
                .FirstOrDefaultAsync();
            ...
            var nicknameCheck = await _context.Developer
                .Where(du => du.Nickname == userRegDto.Nickname)
                .FirstOrDefaultAsync();
            if (nicknameCheck != null) return new RegistrationDto()
            {
            ...
            var bu = await _context.AddAsync(new BaseUser()
            {
                ...
                Salt = salt,
                Hash = salt + userRegDto.Password,
                Photo = _basePhoto,
            });
            await _context.SaveChangesAsync();
            await _context.AddAsync(new Developer()
            {
                UserId = bu.Entity.Id,
                Name = userRegDto.Name,
                Surname = userRegDto.Surname,
                Nickname = userRegDto.Nickname.ToLower(),
                BirthDate = userRegDto.BirthDate
            });
            await _context.SaveChangesAsync();
            return new RegistrationDto()
            {
                ErrorCode = (int) CreateUserErrorType.Ok,
                Url = userRegDto.Nickname
            };
        }
